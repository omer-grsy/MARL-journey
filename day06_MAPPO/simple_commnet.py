...
# 4. ROLLOUT
# =========================
def collect_rollout(env, policy, adj, agents, device, rollout_steps=ROLLOUT_STEPS):
    ...
    for i, agent in enumerate(agents):
        ...
        if obs and agent in obs:
            ...
            with torch.no_grad():
                _, last_values = policy(last_obs_all, adj)
...
# 5. PPO UPDATE
# =========================
def ppo_update(policy, optimizer, buf, agents, adj, device,
    ...
    for _ in range(ppo_epochs):
        ...
        for start in range(0, T, mini_batch_size):
            ...
            # Her ajanın mini-batch obs'u → [n_agents, mini_batch, obs_dim]
            obs_agents = torch.stack([
                obs[i * T:(i + 1) * T][idx_t]
                for i in range(n_agents)
            ])

            logits, values = policy(obs_agents, adj)
            # logits: [n_agents, mini_batch, act_dim]
            # values: [n_agents, mini_batch]

            logits_flat = logits.reshape(-1, logits.shape[-1])
            values_flat = values.reshape(-1)

            # Flat buffer'da doğru indexler
            all_idx = torch.cat([
...
def plot_rewards(reward_history, n_agents):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("reward_curve_commnet-2.png", dpi=150)
    plt.show()
...
def main():
    ...
    for update in range(TOTAL_UPDATES):
        buf = collect_rollout(env, policy, ADJ, agents, device)
        ppo_update(policy, optimizer, buf, agents, ADJ, device)
        # scheduler.step()
        mean_reward = np.mean([
